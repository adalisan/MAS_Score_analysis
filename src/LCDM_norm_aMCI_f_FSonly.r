...
pdf(file=paste("test_results_aMCI_f_FSonly/summarystat_",REGION,".pdf",sep=''))

col=c('black','red','blue','orange','green','pink')
thk95<-c()
thk99<-c()
vol95<-c()
vol99<-c()
for(i in 1:length(fileNames)){ 
	if (i==1) 
           hist(distsl[[i]], 100, freq=FALSE, col="white", border="white", xlim=c(-2,8), ylim=c(0,.4),main=NA,xlab="Distance (mm)",ylab="Probability Density")
    	   par(new=T);
...
legend(3.2, .35, c('normal (FSonly)','nMCI (FSonly)','aMCI (FSonly)','normal (FSLCDM Pipeline)','nMCI (FSLCDM Pipeline)','aMCI (FSLCDM Pipeline)'), cex=1,  col= c('black','red','blue','orange','green','pink'),lwd=2, bty="n");

for(i in 1:length(fileNames)){ 
#	cdf1=ecdf(sample(distsl[[i]],100000,replace=F))
...
legend(2.5, .4, c('normal (FSonly)','nMCI (FSonly)','aMCI (FSonly)','normal (FSLCDM Pipeline)','nMCI (FSLCDM Pipeline)','aMCI (FSLCDM Pipeline)'), cex=1,  col= c('black','red','blue','orange','green','pink'),lwd=2, bty="n");
...
file1=paste("pooled_antsy_files_FSonly/",REGION,"_normal_f_wave1_pooled_antsy.txt",sep='')
file2=paste("pooled_antsy_files_FSonly/",REGION,"_nonamnesticMCI_f_wave1_pooled_antsy.txt",sep='')
file3=paste("pooled_antsy_files_FSonly/",REGION,"_amnesticMCI_f_wave1_pooled_antsy.txt",sep='')
file4=paste("pooled_antsy_files/",REGION,"_normal_f_wave1_pooled_antsy.txt",sep='')
file5=paste("pooled_antsy_files/",REGION,"_nonamnesticMCI_f_wave1_pooled_antsy.txt",sep='')
file6=paste("pooled_antsy_files/",REGION,"_amnesticMCI_f_wave1_pooled_antsy.txt",sep='')

fileNames=c(file1,file2,file3,file4,file5,file6)
...
OUTFILE=paste("/cis/project/sydney/pooled_analysis/test_results_aMCI_f_FSonly/result_",REGION,".txt",sep='')
...
if(length(out)!=0)
	...
	if(b[2]){
		...
		cat("D for KS Test (two-sided, less, greater): \t", out[[10]]$statistic,"\t",out[[11]]$statistic,"\t",out[[12]]$statistic,"\n",file=OUTFILE,append=TRUE)
		cat("p-value for KS Test, norm-aMCI (two-sided, greater, less): \t",out[[13]]$p.value,"\t",out[[14]]$p.value,"\t",out[[15]]$p.value,"\n",file=OUTFILE,append=TRUE)
		cat("D for KS Test (two-sided, less, greater): \t", out[[13]]$statistic,"\t",out[[14]]$statistic,"\t",out[[15]]$statistic,"\n",file=OUTFILE,append=TRUE)
		cat("p-value for KS Test, nMCI-aMCI (two-sided, greater, less): \t",out[[16]]$p.value,"\t",out[[17]]$p.value,"\t",out[[18]]$p.value,"\n",file=OUTFILE,append=TRUE)
		cat("D for KS Test (two-sided, less, greater): \t", out[[16]]$statistic,"\t",out[[17]]$statistic,"\t",out[[18]]$statistic,"\n",file=OUTFILE,append=TRUE)
		}
	if(b[3]){
