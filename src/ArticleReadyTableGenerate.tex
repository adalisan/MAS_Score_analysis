...
{\ttfamily\noindent\itshape\color{messagecolor}{\#\# The following object is masked from 'package:dplyr':\\\#\# \\\#\#\ \ \ \  recode}}\begin{verbatim}
...
\begin{kframe}
\begin{alltt}
  \hlstd{cond_1} \hlkwb{=} \hlstd{conditions[comparisons[,}\hlnum{1}\hlstd{]]}
  \hlstd{cond_2} \hlkwb{=} \hlstd{conditions[comparisons[,}\hlnum{2}\hlstd{]]}
  \hlstd{comparison_strings} \hlkwb{<-} \hlkwd{paste}\hlstd{(cond_1,cond_2,} \hlkwc{sep} \hlstd{=}\hlstr{" vs "}\hlstd{)}
  \hlstd{test_names} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlstr{"KS test"}\hlstd{,}\hlstr{"MWU test"}\hlstd{,} \hlstr{"Welch's t-test"}\hlstd{)}
  \hlstd{col_names} \hlkwb{<-} \hlkwd{outer}\hlstd{(hypothesis_ordinality,test_names,paste,}  \hlkwc{sep}\hlstd{=}\hlstr{"."}\hlstd{)}
  \hlstd{p_value_table_names} \hlkwb{<-}\hlkwd{expand.grid}\hlstd{(comparison_strings, col_names)}
  \hlstd{p_value_table} \hlkwb{<-} \hlkwd{matrix}\hlstd{(}\hlnum{0}\hlstd{,}\hlkwc{nrow} \hlstd{=} \hlnum{7}\hlstd{,}\hlkwc{ncol} \hlstd{=} \hlnum{9}\hlstd{)}

  \hlkwa{for} \hlstd{(side} \hlkwa{in} \hlstd{sides)\{}
    \hlkwa{for} \hlstd{( i} \hlkwa{in} \hlnum{1}\hlopt{:}\hlnum{7} \hlstd{) \{}
      \hlkwa{for} \hlstd{(j} \hlkwa{in} \hlnum{1}\hlopt{:}\hlnum{3}\hlstd{)\{}
        \hlstd{test.type} \hlkwb{<-}  \hlstd{hypothesis_ordinality[j]}
        \hlkwa{for} \hlstd{(k} \hlkwa{in} \hlnum{0}\hlopt{:}\hlnum{2}\hlstd{)\{}
          \hlstd{test.name} \hlkwb{<-} \hlkwd{names}\hlstd{(pooled_test_results[[i]][[test.type]][(k}\hlopt{+}\hlnum{1}\hlstd{)])}
        \hlstd{p_value_table[i,j} \hlopt{+} \hlstd{(k}\hlopt{*}\hlnum{3}\hlstd{)]} \hlkwb{<-} \hlstd{pooled_test_results[[i]][[test.type]][[test.name]]}\hlopt{$}\hlstd{p.value}
    ...
    \hlstd{\}}
  \hlstd{\}}
  \hlkwd{colnames}\hlstd{(p_value_table)}\hlkwb{<-} \hlstd{col_names}
  \hlstd{xtab.out} \hlkwb{<-} \hlkwd{xtable}\hlstd{(p_value_table)}
  \hlstd{addtorow} \hlkwb{<-} \hlkwd{list}\hlstd{()}
  \hlstd{addtorow}\hlopt{$}\hlstd{pos} \hlkwb{<-} \hlkwd{list}\hlstd{(}\hlnum{0}\hlstd{)}
  \hlstd{addtorow}\hlopt{$}\hlstd{command} \hlkwb{<-} \hlkwd{paste0}\hlstd{(}\hlkwd{paste0}\hlstd{(}\hlstr{'& \textbackslash{}\textbackslash{}multicolumn\{3\}\{c\}\{'}\hlstd{,} \hlkwd{c}\hlstd{(}\hlstr{"KS test"}\hlstd{,}\hlstr{"MWU test"}\hlstd{,} \hlstr{"Welch's t-test"}\hlstd{),} \hlstr{'\}'}\hlstd{,} \hlkwc{collapse}\hlstd{=}\hlstr{''}\hlstd{),} \hlstr{'\textbackslash{}\textbackslash{}\textbackslash{}\textbackslash{}'}\hlstd{)}
  \hlkwd{print} \hlstd{(xtab.out,} \hlkwc{add.to.row} \hlstd{= addtorow,} \hlkwc{only.contents}\hlstd{=}\hlnum{TRUE}\hlstd{,}
...
two.sided.KS test & less.KS test & greater.KS test & two.sided.MWU test & less.MWU test & greater.MWU test & two.sided.Welch's t-test & less.Welch's t-test & greater.Welch's t-test \\ 
  & \multicolumn{3}{c}{KS test}& \multicolumn{3}{c}{MWU test}& \multicolumn{3}{c}{Welch's t-test}\\ \hline
