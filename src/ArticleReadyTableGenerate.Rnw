...
<<Pooled_Stat_Tests,cache=TRUE>>=
  ...
  pool_stats_all <-  matrix(rep(c(),10), nrow=2,ncol=5,dimnames = list(sides=sides,regions=regions))
  for ( side in sides) {
    ...
    for ( region in regions ) {
      ...
      for ( i in 1:7 ) {
        ...
        pooled_test_results[[i]] <-list(two.sided=c(),less=c(),greater=c())
    ...
    pool_stat_tests_all_regions_sides[side,region] <- c(pool_stat_tests_all_regions_sides[side,region],pooled_test_results)
  }
}
