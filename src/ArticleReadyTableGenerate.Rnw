...
<<echo=FALSE, results='asis',cache=FALSE >>=
  for (region in regions) { 
    ...
    rowcom <- list()
    rowcom$pos = list(-1)
    rowcom$command = paste0(paste0("Region",paste0(" & \\\\multicolumn{3}{c}{", col.names , '}', collapse=''),collapse='')," \\\\ " ,collapse="")
    ...
    xtablelist.out<-print(xtablelist, only.contents=FALSE,
      ...
      type="latex",floating.environment = "sidewaystable", print.results = FALSE)
