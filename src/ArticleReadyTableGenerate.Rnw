...
<<LoadData_and_BasicDataMunge,echo=FALSE,results='hide',warning=FALSE, message=FALSE>>=
# Load Data
library(foreign)
library(dplyr)
library(xtable)
...
source("./src/MAS_MCIsubtype_analysis.R")
...
<<Table_1_summarystats, results='asis', eval=TRUE,echo=FALSE>>=
...
<<Pooled_Stat_Tests>>=
 ...
 side =  c("lh","rh")
  ...
  comparisons = t(array(1,2,1,3,1,4,1,5,2,3,2,4,4,5,dim = c(2,7)))
  pooled_stats_list = list()
  
  #read the pooled files 
  for (i in 1:5) {
    cond_fileh <- open(file = paste0(DATA_DIR,"/",side,'_',region,"_", conditions[i], "_wave1_pooled_antsy.txt"))
    pooled_stats_list <-read.table(cond_fileh, nrows = 2.5E6, colClasses = "numeric")
    if ( is.data.frame(pooled_stats_list) ) {
      pooled_stats_list <- pooled_stats_list[,1]
    }
    pooled_stats_list <-c(pooled_stats_list, list(pool_stats)) 
  }
