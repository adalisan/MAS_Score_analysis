...
for (n in 1:HISTNUMB) {
    ...
    percent_above2<-c()
    for (i in 1:ROWS){
    	fn1=paste("/cis/project/sydney/MAS.20100921_done/",names[[1]][i],"/FSLCDM_v2.3/",REGION,"/",SUBREGION,"_orig_1_AntsyGrey.txt",sep="")  #create path of individual files here 
#	fn2=paste("/cis/project/sydney/MAS.20100921_done/",names[[1]][i],"/FSLCDM_v2.3/",REGION,"/",SUBREGION,"_orig_0.5_AntsyGrey.txt",sep="")
	fn3=paste("/cis/project/sydney/MAS.20100921_done/",names[[1]][i],"/FSLCDM_v2.3/",REGION,"/",SUBREGION,"_1_AntsyGrey.txt",sep="")
#	fn4=paste("/cis/project/sydney/MAS.20100921_done/",names[[1]][i],"/FSLCDM_v2.3/",REGION,"/",SUBREGION,"_0.5_AntsyGrey.txt",sep="")
    	xdata1=scan(fn1)
#	xdata2=scan(fn2)
    	xdata3=scan(fn3)
#	xdata4=scan(fn4)
	if (i==1) 
    	   ...
    	   par(new=T);
    	lines(density(xdata1), lwd=2, col=col[i], lty=4)
#    	lines(density(xdata2), lwd=2, col=col[i], lty=3)
	lines(density(xdata3), lwd=2, col=col[i])
#	lines(density(xdata4), lwd=2, col=col[i], lty=2)

	percent_above1[i]=round(sum(xdata1>4)/length(xdata1),4)
	percent_above2[i]=round(sum(xdata3>4)/length(xdata3),4)
